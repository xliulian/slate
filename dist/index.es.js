import React, { useLayoutEffect, useEffect, useRef, useContext, createContext, useMemo, useCallback, useState } from 'react';
import { Editor, Operation, Path, Node as Node$1, Text as Text$1, Range, Element as Element$1, Transforms } from 'slate';
...
var MemoizedLeaf = /*#__PURE__*/React.memo(Leaf, (prev, next) => {
  return next.parent === prev.parent && next.isLast === prev.isLast && next.renderLeaf === prev.renderLeaf && next.text === prev.text && isEqual(next.leaf, prev.leaf);
});
...
var MemoizedElement = /*#__PURE__*/React.memo(Element, (prev, next) => {
  return prev.element === next.element && prev.renderElement === next.renderElement && prev.renderLeaf === next.renderLeaf && isRangeListEqual$1(prev.decorations, next.decorations) && (prev.selection === next.selection || !!prev.selection && !!next.selection && Range.equals(prev.selection, next.selection));
});
/**
...
var isRangeListEqual$1 = (list, another) => {
  if (list.length !== another.length) {
    return false;
  }
...
var Children = props => {
  ...
  var nodeChildren = isLeafBlock || !node.childrenOrderedBy ? node.children.map((child, idx) => [child, idx]) : (node.childrenOrderedBy.length === 2 && (['asc', 'desc'].includes(node.childrenOrderedBy[1]) || Array.isArray(node.childrenOrderedBy[1]) && ['asc', 'desc'].includes(node.childrenOrderedBy[1][0])) ? orderBy(node.children, node.childrenOrderedBy[0], node.childrenOrderedBy[1]) : orderBy(node.children, node.childrenOrderedBy)).map(child => [child, node.children.indexOf(child)]);
  ...
  for (var child of nodeChildren) {
    ...
    if (Element$1.isElement(n)) {
      if (node.droppable) {
        children.push( /*#__PURE__*/React.createElement(IndexContext.Provider, {
          key: key.id,
          value: i
...
var Editable = props => {
  ...
  var onDOMSelectionChange = useCallback(() => setTimeout(throttle(() => {
  ...
  var decorateContextValue = useMemo(() => ({
    decorate,
    useDecorateState
...
var withReact = editor => {
  ...
  e.apply = op => {
    ...
    switch (op.type) {
      ...
      case 'set_node':
        {
          for (var [_node, _path] of Editor.levels(e, {
            at: op.newProperties.hasOwnProperty('type') ? Path.parent(op.path) : op.path
          })) {
      ...
      case 'move_node':
        {
          for (var [_node3, _path3] of Editor.levels(e, {
    ...
    for (var [_path4, _key4] of matches) {
      var [_node4] = Editor.node(e, _path4);
...
export { DefaultElement, DefaultLeaf, Editable, Hotkeys, ReactEditor, Slate, useEditor, useFocused, useIndex, useReadOnly, useSelected, useSlate, useSlateStatic, withReact };
