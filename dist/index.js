  ...
  e.apply = function (op) {
    ...
    if (save) {
      ...
      if (lastBatch && merge) {
        ...
        if (overwrite) {
          lastBatch.pop(); // XXX: only set_selection op do override, and we do it carefully also for the incomplete set_selection case.

          if (op.type === 'set_selection' && (lastOp === null || lastOp === void 0 ? void 0 : lastOp.type) === 'set_selection' && op.newProperties) {
            mergedOp = _objectSpread(_objectSpread({}, lastOp), {}, {
              newProperties: _objectSpread(_objectSpread({}, lastOp.newProperties), op.newProperties)
            });
          }
        }
