...
var MemoizedLeaf = /*#__PURE__*/React__default['default'].memo(Leaf, function (prev, next) {
  return next.parent === prev.parent && next.isLast === prev.isLast && next.renderLeaf === prev.renderLeaf && next.text === prev.text && isEqual__default['default'](next.leaf, prev.leaf);
});
...
var MemoizedText = /*#__PURE__*/React__default['default'].memo(Text, function (prev, next) {
  return next.parent === prev.parent && next.isLast === prev.isLast && next.renderLeaf === prev.renderLeaf && next.text === prev.text && isRangeListEqual(prev.decorations, next.decorations);
});
/**
...
var isRangeListEqual = function isRangeListEqual(list, another) {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    if (!isEqual__default['default'](range, other)) {
      return false;
    }
  }
...
var Element = function Element(props) {
  ...
  var children; // Attributes that the developer must mix into the element in their
  ...
  if (slate.Editor.isVoid(editor, element)) {
    ...
    children = /*#__PURE__*/React__default['default'].createElement(Children, {
...
var MemoizedElement = /*#__PURE__*/React__default['default'].memo(Element, function (prev, next) {
  return prev.element === next.element && prev.renderElement === next.renderElement && prev.renderLeaf === next.renderLeaf && isRangeListEqual$1(prev.decorations, next.decorations) && (prev.selection === next.selection || !!prev.selection && !!next.selection && slate.Range.equals(prev.selection, next.selection));
});
/**
...
var isRangeListEqual$1 = function isRangeListEqual(list, another) {
  if (list.length !== another.length) {
    return false;
  }
...
var Children = function Children(props) {
  ...
  var _useDecorate = useDecorate(),
      decorate = _useDecorate.decorate,
      useDecorateState = _useDecorate.useDecorateState;

  var decorateState = useDecorateState && useDecorateState();
  ...
  var nodeChildren = isLeafBlock || !node.childrenOrderedBy ? node.children.map(function (child, idx) {
    return [child, idx];
  }) : (node.childrenOrderedBy.length === 2 && (['asc', 'desc'].includes(node.childrenOrderedBy[1]) || Array.isArray(node.childrenOrderedBy[1]) && ['asc', 'desc'].includes(node.childrenOrderedBy[1][0])) ? orderBy__default['default'](node.children, node.childrenOrderedBy[0], node.childrenOrderedBy[1]) : orderBy__default['default'](node.children, node.childrenOrderedBy)).map(function (child) {
    return [child, node.children.indexOf(child)];
  });
  ...
  try {
    for (_iterator.s(); !(_step = _iterator.n()).done;) {
      ...
      if (slate.Element.isElement(n)) {
        if (node.droppable) {
          children.push( /*#__PURE__*/React__default['default'].createElement(IndexContext.Provider, {
            key: key.id,
            value: i
...
var IS_WECHAT = typeof navigator !== 'undefined' && /MicroMessenger/i.test(navigator.userAgent); // Check if DOM is available as React does internally.
...
var getDomPath = function getDomPath(el) {
  if (!el) {
    return;
  }
  ...
  while (el.parentNode !== null) {
    ...
    for (var i = 0; i < el.parentNode.childNodes.length; i += 1) {
    ...
    var nodeName = CSS.escape(el.nodeName.toLowerCase());
    ...
    if (nodeName === 'html') break; // if ( el.hasAttribute('id') && el.id != '' ) { no id shortcuts, ids are not unique in shadowDom
    //   stack.unshift(`#${CSS.escape(el.id)}`);
    // } else

    if (sibCount > 1) {
      stack.unshift("".concat(nodeName, ":nth-of-type(").concat(sibIndex + 1, ")"));
    } else {
...
var HAS_BEFORE_INPUT_SUPPORT = !IS_CHROME_LEGACY && !IS_EDGE_LEGACY && !IS_WECHAT && globalThis.InputEvent && // @ts-ignore The `getTargetRanges` property isn't recognized.
...
var Editable = function Editable(props) {
  var autoFocus = props.autoFocus,
      ...
      useDecorateState = props.useDecorateState,
      ...
      attributes = _objectWithoutProperties(props, ["autoFocus", "decorate", "useDecorateState", "onDOMBeforeInput", "placeholder", "readOnly", "renderElement", "renderLeaf", "scrollSelectionIntoView", "style", "as"]);
  ...
  var onDOMSelectionChange = React.useCallback(function () {
    return setTimeout(throttle__default['default'](function () {
  ...
  }, [readOnly]); // Attach a native DOM event handler for `selectionchange`, because React's
  ...
  var decorateContextValue = React.useMemo(function () {
    return {
      decorate: decorate,
      useDecorateState: useDecorateState
    };
...
var ReactEditor = {
  ...
  toDOMRange: function toDOMRange(editor, range) {
    ...
    var isCollapsed = slate.Range.isCollapsed(range);
    var domFocus = isCollapsed ? domAnchor : ReactEditor.toDOMPoint(editor, focus);
  ...
  toSlateNode: function toSlateNode(editor, domNode) {
    ...
    if (!node) {
      console.warn('Cannot resolve a Slate node from DOM node:', domEl, "".concat(getDomPath(domEl)));
      throw new Error("Cannot resolve a Slate node from DOM node: ".concat(domEl, " ").concat(getDomPath(domEl)));
    }
  ...
  toSlatePoint: function toSlatePoint(editor, domPoint, exactMatch) {
    ...
    if (!textNode) {
      ...
      console.warn('Cannot resolve a Slate point from DOM point:', domPoint, "".concat(getDomPath(domPoint[0])));
      throw new Error("Cannot resolve a Slate point from DOM point: ".concat(domPoint, " ").concat(getDomPath(domPoint[0])));
...
var withReact = function withReact(editor) {
  ...
  e.apply = function (op) {
    ...
    switch (op.type) {
      ...
      case 'set_node':
        {
          var _iterator2 = _createForOfIteratorHelper$1(slate.Editor.levels(e, {
            at: op.newProperties.hasOwnProperty('type') ? slate.Path.parent(op.path) : op.path
          })),
              _step2;
      ...
      case 'move_node':
        {
          var _iterator4 = _createForOfIteratorHelper$1(slate.Editor.levels(e, {
          ...
          try {
            for (_iterator4.s(); !(_step4 = _iterator4.n()).done;) {
              var _step4$value = _slicedToArray(_step4.value, 2),
                  _node3 = _step4$value[0],
                  _path3 = _step4$value[1];

              var _key3 = ReactEditor.findKey(e, _node3);

              matches.push([_path3, _key3]);
            }
          } catch (err) {
            _iterator4.e(err);
          } finally {
    ...
    for (var _i = 0, _matches = matches; _i < _matches.length; _i++) {
      var _matches$_i = _slicedToArray(_matches[_i], 2),
          _path4 = _matches$_i[0],
          _key4 = _matches$_i[1];

      var _Editor$node = slate.Editor.node(e, _path4),
          _Editor$node2 = _slicedToArray(_Editor$node, 1),
          _node4 = _Editor$node2[0];

      NODE_TO_KEY.set(_node4, _key4);
    }
  };
  ...
  e.insertData = function (data) {
    ...
    if (text) {
      ...
      var _iterator5 = _createForOfIteratorHelper$1(lines),
          _step5;
